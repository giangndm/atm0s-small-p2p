...
async fn sync_must_not_block_connection_task_on_full_main_queue() {
    ...
    let conn = node1.ctx.conns().into_iter().next().expect("node1 should have a connection to node2");
    let route = SharedRouterTable::new(addr1.peer_id()).create_sync(&addr2.peer_id());
    let advertise = PeerDiscovery::new(vec![]).create_sync_for(100, &addr2.peer_id());
    conn.try_send(PeerMessage::Sync { route, advertise }).expect("sync should enqueue to peer connection");

    tokio::time::sleep(Duration::from_millis(100)).await;

    let send = tokio::time::timeout(Duration::from_millis(500), service1_requester.send_unicast(addr2.peer_id(), b"after-sync".to_vec())).await;
    let delivered = tokio::time::timeout(Duration::from_millis(500), service2.recv()).await;

    assert!(
        matches!(send, Ok(Ok(()))) && delivered == Ok(Some(P2pServiceEvent::Unicast(addr1.peer_id(), b"after-sync".to_vec()))),
        "inbound Sync handling must not block later traffic on the connection when the main event queue is full; send={send:?}, delivered={delivered:?}"
    );
