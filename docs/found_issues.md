...
- Fix status: fixed by using checked `FetchChanged` range arithmetic at the
...
- Status: fixed. Collectors now open an expected-responder set from current
...
- Fix status: fixed by validating the `SubscriberHandleId` carried by
  `InternalMsg::FeedbackRpc` against the channel's live local subscribers
  before delivering to publishers or creating pending RPC state.
...
- Fix status: `MAX_RPC_METHOD_LEN = 1024` is enforced for inbound
  ...
  focused evidence now seeds valid remote membership for `PublishRpc` so it
  exercises method validation instead of the non-publisher authorization path.
...
- Score: 68/100
- Fix status: fixed by making `PeerMemory::select_best` prefer direct paths
  over relayed candidates whenever a direct path exists for the destination.
...
- Result: no accepted non-duplicate issue.
- Reviewer: `Avicenna the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/router.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=341 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_valid_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=341, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 26 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
  - one stale-route panic at `src/router.rs:76:66` with
    `should have direct metric with apply_sync`.
  - invalid-service, shutdown-send, PeerStopped storm, no-capacity,
    channel-closed, broadcast-failure, endpoint-driver-dropped, and
    internal-channel-error signatures were absent.
  - lifecycle context included five `connection lost` markers and one
    `closed by peer` marker; reviewer treated these as churn context, not as
    separate issue evidence.
- Duplicate mapping: ISSUE-063 for stale `PeerData::Sync` after a direct route
  was removed.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  failure as the already accepted stale-sync route panic.
- Smallest fix proposal: no summary fix change; keep ISSUE-063 fix proposal to
  guard or drop stale sync when the direct metric is missing and clear queued
  sync state on direct route removal.
...
- Result: no accepted non-duplicate issue.
- Reviewer: `Bohr the 7th`, forked subagent review, confirmed duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=339 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_sanitized_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=339, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 21 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
  - one incoming shutdown-send panic at `src/peer.rs:92:104` with
  ...
  - invalid-service, stale-route, PeerStopped storm, no-capacity,
    ...
    signatures were absent.
  - one `aborted by peer` connection-error marker was reviewed as lifecycle
    context.
- Duplicate mapping: ISSUE-139 for the shutdown/closed-main reporting panic.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  panic as the already accepted incoming `PeerConnectError` reporting race
...
### Cycle after ISSUE-204 no-new cycle 338: steady valid clean pass

- Result: no accepted non-duplicate issue.
- Reviewer: `Heisenberg the 7th`, forked subagent review, confirmed
  clean/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=338 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5600 cargo test fuzz_random_steady_valid_node_actions_must_not_panic_connection_tasks -- --nocapture`
    passed with `1 passed`, `0 failed`.
- Evidence summary:
  - exit status 0; log had 8 lines; test result was `ok. 1 passed; 0 failed;
    0 ignored; 0 measured; 289 filtered out; finished in 36.35s`.
  - invalid-service, stale-route, shutdown-send, PeerStopped storm,
    connection-lifecycle, channel-closed, endpoint-driver-dropped,
    internal-channel-error, and panic signatures were absent.
- Duplicate mapping: none.
- Root-cause summary impact: no new root cause.
- Smallest fix proposal: none for this cycle.

### Cycle after ISSUE-204 no-new cycle 337: valid churn outgoing shutdown-send duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Jason the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=337 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_valid_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=337, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 21 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
...
- Duplicate mapping: ISSUE-139 for the shutdown/closed-main reporting panic.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  panic as the already accepted outgoing `PeerConnectError` reporting race
  after main-loop shutdown.
- Smallest fix proposal: no summary fix change; keep ISSUE-139 fix proposal to
  replace shutdown-path `expect("should send to main")` calls at
  `src/peer.rs:89/92/130/133` with graceful closed-channel handling.

### Cycle after ISSUE-204 no-new cycle 336: broad random invalid-service and shutdown-send duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Bernoulli the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/ctx.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=336 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_node_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=336, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 48 lines; the fuzz assertion at
    `src/tests/fuzz.rs:183:5` reported background connection/service task
    failure.
  ...
  - stale-route, PeerStopped storm, no-capacity, broadcast-failure,
    ...
    absent.
  - lifecycle context included one `connection lost`, five `closed by peer`,
    six try-send `channel closed`, and three `aborted by peer` markers.
- Duplicate mapping: ISSUE-053 for unchecked inbound service-id table
...
- Root-cause summary impact: no new root cause; reviewer classified all hard
  failure signatures as already accepted invalid-service and shutdown-send
  failures.
- Smallest fix proposal: no summary fix change; keep ISSUE-053 fix proposal to
  validate decoded `P2pServiceId` before indexing the fixed service table and
...
### Cycle after ISSUE-204 no-new cycle 335: sanitized churn outgoing shutdown-send duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Ptolemy the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=335 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_sanitized_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=335, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 76 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
  - one outgoing shutdown-send panic at `src/peer.rs:133:113` with
    `should send to main: SendError { .. }`.
  - invalid-service, stale-route, PeerStopped storm, no-capacity,
    channel-closed, endpoint-driver-dropped, and internal-channel-error
    signatures were absent.
  - lifecycle context included 12 `connection lost`, 21 closed-by-peer
...
### Cycle after ISSUE-204 no-new cycle 334: steady valid clean pass

- Result: no accepted non-duplicate issue.
- Reviewer: `Newton the 7th`, forked subagent review, confirmed clean/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=334 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5600 cargo test fuzz_random_steady_valid_node_actions_must_not_panic_connection_tasks -- --nocapture`
    passed with `1 passed`, `0 failed`.
- Evidence summary:
  - exit status 0; log had 8 lines; test result was `ok. 1 passed; 0 failed;
    0 ignored; 0 measured; 289 filtered out; finished in 36.20s`.
  - invalid-service, stale-route, shutdown-send, PeerStopped storm,
    connection-lifecycle, channel-closed, endpoint-driver-dropped,
    internal-channel-error, and panic signatures were absent.
- Duplicate mapping: none.
...
### Cycle after ISSUE-204 no-new cycle 333: valid churn stale-sync and stop-pressure duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Bacon the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/router.rs`
  - `src/peer/peer_internal.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=333 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_valid_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=333, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 1,014 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
  - one stale-sync panic at `src/router.rs:76:66` with
    `should have direct metric with apply_sync`.
  - 986 forwarded-stop alias error logs, all `no available capacity` markers.
  - invalid-service, shutdown-send, broadcast-failure, channel-closed,
    endpoint-driver-dropped, and internal-channel-error signatures were
    absent.
  - lifecycle context included one `connection lost`, two `closed by peer`,
    and two `aborted by peer` markers.
- Duplicate mapping: ISSUE-063 for stale sync after direct-route removal;
  ISSUE-170 for PeerStopped forwarding storm/backpressure pressure.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  failure as the already accepted stale-sync panic and the no-capacity burst as
  existing stopped-forwarding backpressure evidence.
- Smallest fix proposal: no summary fix change; keep ISSUE-063 fix proposal to
  replace the direct-metric `expect` with a stale-sync guard and clear queued
  sync state when direct routes are removed; keep ISSUE-170 fix proposal to
  dedupe/coalesce stopped forwarding and add bounded retry/backpressure plus
  TTL/tombstone suppression.

### Cycle after ISSUE-204 no-new cycle 332: broad random stale-sync and stop-storm duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Faraday the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/router.rs`
  - `src/peer/peer_internal.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=332 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_node_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=332, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 4,117 lines; the fuzz assertion at
    `src/tests/fuzz.rs:183:5` reported background connection/service task
    failure.
  - two stale-sync panics at `src/router.rs:76:66` with
    `should have direct metric with apply_sync`.
  - 4,090 forwarded-stop alias error logs, including 3,072
    `no available capacity` markers and 1,022 `channel closed` markers.
  - invalid-service, shutdown-send, broadcast-failure,
    endpoint-driver-dropped, and internal-channel-error signatures were
    absent.
  - one closed-by-peer marker was reviewed as churn context.
- Duplicate mapping: ISSUE-063 for stale sync after direct-route removal;
  ISSUE-170 for PeerStopped forwarding storm/backpressure failure.
- Root-cause summary impact: no new root cause; reviewer classified the router
  panic and forwarded-stop storm as already accepted issues.
- Smallest fix proposal: no summary fix change; keep ISSUE-063 fix proposal to
  guard/drop stale sync when the direct metric is gone and clear queued sync
  state when direct routes are removed; keep ISSUE-170 fix proposal to
  dedupe/coalesce `PeerStopped` forwarding with bounded retry/backpressure and
  TTL/tombstone suppression.
...
- Result: no accepted non-duplicate issue.
- Reviewer: `Mill the 7th`, forked subagent review, confirmed clean/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=330 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5600 cargo test fuzz_random_steady_valid_node_actions_must_not_panic_connection_tasks -- --nocapture`
    passed with `1 passed`, `0 failed`.
- Evidence summary:
  - exit status 0; log had 8 lines; test result was `ok. 1 passed; 0 failed;
    0 ignored; 0 measured; 289 filtered out; finished in 36.23s`.
  - invalid-service, stale-route, shutdown-send, PeerStopped storm,
    connection-lifecycle, channel-closed, endpoint-driver-dropped,
    internal-channel-error, and panic signatures were absent.
- Duplicate mapping: none.
...
- Result: no accepted non-duplicate issue.
- Reviewer: `Einstein the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=327 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_sanitized_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=327, nodes=8, steps=5200`.
- Evidence summary:
  ...
  - invalid-service, stale-route, PeerStopped storm, no-capacity,
    channel-closed, endpoint-driver-dropped, and internal-channel-error
    signatures were absent.
  - network churn context included three `aborted by peer` connection errors
    to `PeerId(7)`.
- Duplicate mapping: ISSUE-139 for the shutdown/closed-main reporting panic.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  failure as the already accepted incoming `PeerConnectError` reporting race
  after main-loop shutdown.
- Smallest fix proposal: no summary fix change; keep ISSUE-139 fix proposal to
  replace shutdown-path `expect("should send to main")` calls at
  `src/peer.rs:89/92/130/133` with graceful closed-channel handling.

### Cycle after ISSUE-204 no-new cycle 326: valid churn shutdown-send duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Wegener the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/peer.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=326 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5200 cargo test fuzz_random_valid_node_churn_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=326, nodes=8, steps=5200`.
- Evidence summary:
  - exit status 101; log had 31 lines; the fuzz assertion at
    `src/tests/fuzz.rs:372:5` reported background connection/service task
    failure.
  - one hard shutdown-send panic at `src/peer.rs:130:121` with
    `should send to main: SendError { .. }`.
  - invalid-service, stale-route, PeerStopped storm, channel-closed,
    endpoint-driver-dropped, and internal-channel-error signatures were
    absent.
  - network churn context included one `connection lost`, five
    `closed by peer`, and one `aborted by peer` marker.
- Duplicate mapping: ISSUE-139 for the shutdown/closed-main reporting panic.
- Root-cause summary impact: no new root cause; reviewer classified the hard
  failure as one of the already accepted closed-main shutdown send sites.
- Smallest fix proposal: no summary fix change; keep ISSUE-139 fix proposal to
  replace shutdown-path `expect("should send to main")` calls at
  `src/peer.rs:89/92/130/133` with graceful closed-channel handling.

### Cycle after ISSUE-204 no-new cycle 325: steady valid clean pass

- Result: no accepted non-duplicate issue.
- Reviewer: `Volta the 7th`, forked subagent review, confirmed clean/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=325 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=5600 cargo test fuzz_random_steady_valid_node_actions_must_not_panic_connection_tasks -- --nocapture`
    passed with `1 passed`, `0 failed`.
- Evidence summary:
  - exit status 0; log had 8 lines; test result was `ok. 1 passed; 0 failed;
    0 ignored; 0 measured; 289 filtered out; finished in 36.44s`.
  - invalid-service, stale-route, shutdown-send, PeerStopped storm,
    connection-lifecycle, channel-closed, endpoint-driver-dropped,
    internal-channel-error, and panic signatures were absent.
- Duplicate mapping: none.
...
### Cycle after ISSUE-204 no-new cycle 324: invalid service-id duplicate

- Result: no accepted non-duplicate issue.
- Reviewer: `Herschel the 7th`, forked subagent review, confirmed
  duplicate/no-new.
- Source and test evidence reviewed:
  - `src/tests/fuzz.rs`
  - `src/ctx.rs`
  - `RUST_LOG=error P2P_FUZZ_SEED=324 P2P_FUZZ_NODES=8 P2P_FUZZ_STEPS=4800 cargo test fuzz_random_node_actions_must_not_panic_connection_tasks -- --nocapture`
    failed with `seed=324, nodes=8, steps=4800`.
- Evidence summary:
  - exit status 101; log had 26 lines; the fuzz assertion at
    `src/tests/fuzz.rs:183:5` reported background connection/service task
    failure.
  - two hard invalid-service panics at `src/ctx.rs:34:9` with
    `index out of bounds: the len is 256 but the index is 256`.
  - stale-route, shutdown-send, PeerStopped storm, endpoint-driver-dropped,
    and internal-channel-error signatures were absent.
  - minor network context included two `channel closed` markers and two
